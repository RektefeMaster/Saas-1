...
function DashboardIcon({ active }: { active?: boolean }) {
  return (
    <svg className={`h-5 w-5 shrink-0 ${active ? "text-emerald-400" : "text-slate-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <aside className="fixed left-0 top-0 z-40 h-screen w-64 border-r border-slate-200 bg-white shadow-sm">
        <div className="flex h-16 items-center border-b border-slate-200 px-6">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <span className="text-lg font-semibold tracking-tight text-slate-900">SaaSRandevu</span>
          </Link>
        </div>
        <nav className="flex h-[calc(100vh-4rem)] flex-col overflow-y-auto p-3">
          <div className="mb-2 px-3 py-1.5 text-xs font-semibold uppercase tracking-wider text-slate-400">
            Yönetim
          </div>
          {navItems.map((item) => {

...
const ChartBar = dynamic(
  ...
  { loading: () => <div className="h-[280px] animate-pulse rounded-xl bg-slate-100 dark:bg-slate-800" /> }
);
    ...
    <div className="space-y-5">
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      className="inline-flex min-h-10 shrink-0 items-center rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 transition-colors duration-150 hover:bg-slate-50 disabled:opacity-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
                    >
      ...
      {hasAppointments && (
        <section className="panel-surface p-5">
          ...
          <ChartBar data={chartData} xKey="gün" bars="Randevu" colors="emerald" height={280} />
        </section>
      )}
            ...
            <ul className="max-h-40 space-y-2 overflow-y-auto scrollbar-thin">
              {reviews.reviews.map((r) => (
                ...
                >
                  ...
                  {r.comment && (
                    <span className="ml-2 text-slate-700 dark:text-slate-300">– {r.comment}</span>
                  )}
                </li>
              ))}
            </ul>
